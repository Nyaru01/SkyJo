...
export default function Dashboard() {
    ...
    const isRehydrated = useGameStore(state => state.isRehydrated);
    ...
    // OLD local state and sync effects removed to prevent loops

    // Sync global activeTab for music control (Local -> Global) - REMOVED

    // LISTEN to global tab changes (Global -> Local) - REMOVED
        ...
        const handleSWMessage = (event) => {
            if (event.data?.type === 'DEEP_LINK') {
                try {
                    ...
                    if (inviteRoom && userProfile?.name && !roomCode) {
                        const cleanCode = inviteRoom.toUpperCase().trim();
                        console.log('[DASH] Deep-link via SW message:', cleanCode);

                        setOnlinePlayerInfo(userProfile.name, userProfile.avatarId);
                        joinRoom(cleanCode);
                        setVirtualScreen('lobby'); // CRITICAL: Set screen to lobby for auto-nav
                        setActiveTab('virtual');
                        toast.success("Rejoint via invitation !", { icon: '🎮' });
                    }
                } catch (err) {
                    console.error('[DASH] Error parsing deep-link:', err);
                }
            }
        };
    ...
    useEffect(() => {
        const checkDeepLink = () => {
            // Use roomCode instead of isInOnlineSession to avoid blocking during state init
            if (roomCode) return;

            const urlParams = new URLSearchParams(window.location.search);
            const inviteRoom = urlParams.get('room');

            if (inviteRoom && userProfile?.name) {
                const cleanCode = inviteRoom.toUpperCase().trim();
                console.log(`[DASH] Deep-link via URL: ${cleanCode}`);

                // Debug Toast
                toast.success(`Debug: Room detected ${cleanCode}`, { icon: '🐞' });
        ...
        // Check on visibility/focus change (e.g. returning from notification)
        window.addEventListener('focus', checkDeepLink);

        // Polling as a fallback (some mobile PWA environments are weird)
        const interval = setInterval(checkDeepLink, 2000);
    ...
    return (
        <div className="min-h-screen relative">
            ...
            {/* LevelUpCelebration handles its own visibility internally via store */}
            <LevelUpCelebration />
            ...
            <AnimatePresence>
                {gameInvitation && (
                    <motion.div
                        ...
                        className="fixed bottom-24 left-1/2 z-[100] bg-slate-900/95 backdrop-blur-2xl border-2 border-emerald-500/50 rounded-[2rem] p-4 shadow-[0_20px_50px_rgba(16,185,129,0.3)] flex items-center gap-4 min-w-[340px] max-w-[92vw]"
                    >
                        <div className="absolute inset-0 bg-gradient-to-tr from-emerald-500/5 via-transparent to-emerald-500/10 rounded-[2rem] pointer-events-none" />

                        <div className="relative w-14 h-14 rounded-2xl bg-emerald-500 flex items-center justify-center shadow-[0_0_20px_rgba(16,185,129,0.4)] animate-breathing shrink-0">
                            <Play className="h-7 w-7 text-white fill-white shadow-sm" />
                            <div className="absolute -inset-1 bg-emerald-500/20 blur-md rounded-2xl -z-10 animate-pulse" />
                        </div>

                        <div className="flex-1 overflow-hidden relative">
                            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400 mb-0.5">Invitation de {gameInvitation.fromName}</p>
                            <p className="text-white font-black text-lg tracking-tight leading-none">Partie en ligne</p>
                        </div>

                        <div className="flex gap-2 shrink-0 relative">
                            <Button
                                size="sm"
                                className="bg-emerald-500 hover:bg-emerald-400 text-white rounded-xl font-black text-xs px-5 h-11 transition-all active:scale-95 shadow-[0_4px_15px_rgba(16,185,129,0.4)] hover:shadow-[0_8px_25px_rgba(16,185,129,0.5)] border-b-4 border-emerald-700 active:border-b-0"
                                onClick={() => {
                            ...
                            <button
                                ...
                                className="w-11 h-11 rounded-xl bg-white/5 hover:bg-white/10 flex items-center justify-center text-slate-400 hover:text-white transition-colors border border-white/10"
                            >
                                <X className="h-6 w-6" />
                            </button>
                        </div>
            ...
            <AnimatePresence>
                {unreadCount > 0 && activeTab !== 'social' && !activeChatId && (
                    <motion.div
                        ...
                        className="fixed bottom-[88px] left-1/2 z-[90] w-[94%] max-w-sm cursor-pointer"
                    >
                        <div className="relative overflow-hidden rounded-[2rem] bg-slate-900/95 backdrop-blur-2xl border-2 border-amber-500/50 p-4 shadow-[0_20px_50px_rgba(245,158,11,0.3)] flex items-center gap-4">
                            <div className="absolute inset-0 bg-gradient-to-tr from-amber-500/5 via-transparent to-amber-500/10 pointer-events-none" />

                            <div className="relative h-14 w-14 flex-shrink-0">
                                <div className="absolute inset-0 bg-amber-500/30 blur-xl animate-pulse" />
                                <div className="relative h-full w-full rounded-2xl bg-gradient-to-br from-amber-400 to-orange-600 flex items-center justify-center shadow-lg animate-breathing">
                                    <Users className="h-7 w-7 text-slate-950" />
                                    <div className="absolute -top-1.5 -right-1.5 h-6 w-6 bg-white rounded-full flex items-center justify-center shadow-xl border-2 border-amber-500">
                                        <span className="text-[11px] font-black text-amber-600">{unreadCount}</span>
                                    </div>
                                </div>
                            </div>

                            <div className="flex-1 min-w-0 relative">
                                <h4 className="text-white font-black text-lg leading-none tracking-tight">Nouveau Message !</h4>
                                <p className="text-amber-400 font-bold text-[10px] uppercase tracking-widest mt-1">Appuie pour lire la conversation</p>
                            </div>

                            <div className="flex items-center justify-center h-11 w-11 rounded-xl bg-amber-500 text-slate-950 shadow-[0_4px_15px_rgba(245,158,11,0.4)] transition-all hover:scale-105 active:scale-95">
                ...
                {isWallpaperModalOpen && (
                    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
                        ...
                        <motion.div
                            ...
                            className="relative w-full max-w-2xl bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-[2rem] shadow-2xl overflow-hidden"
                        >
                                    ...
                                    {WALLPAPERS.map((wp) => (
                                        ...
                                        >
                                            <div className="relative w-full aspect-video rounded-xl overflow-hidden shadow-lg">
                                                <div className={`absolute inset-0 opacity-40 ${wp.color}`} />
                                                <img
                                                    src={wp.url}
                                                    alt={wp.name}
                                                    onError={(e) => e.target.style.display = 'none'}
                                                    className={`w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 ${background === wp.url ? "scale-110" : "scale-100"
                                                        }`}
                                                />
                                                {background === wp.url && (
