...
io.on('connection', (socket) => {
        ...
        if (autoInviteFriendId) {
            ...
            const sendWithRetry = async (retriesLeft) => {
                ...
                sendInvitationNotification(String(effectiveDbId), playerName, stringFriendId, roomCode)
                    .then(res => {
                        if (res.success) {
                            console.log(`[ATOMIC INVITE] Push notification sent to ${stringFriendId}`);
                            socket.emit('invitation_sent', { friendId: stringFriendId, method: 'push' });
                        } else {
                            console.log(`[ATOMIC INVITE] Push notification failed for ${stringFriendId}:`, res.reason);
                    ...
                    .catch(err => {
                        console.error(`[ATOMIC INVITE] Push notification error:`, err);
    ...
    socket.on('start_game', (roomCode) => {
        ...
        io.to(roomCode.toUpperCase()).emit('game_started', { gameState: room.gameState, totalScores: room.totalScores, roundNumber: room.roundNumber });
    });
        ...
        sendInvitationNotification(String(socket.dbId), fromName, friendId, roomCode)
            .then(res => {
                if (res.success) {
                    ...
                    const reason = res.reason === 'No subscription' ? 'PUSH_DISABLED' : 'OFFLINE';
                    socket.emit('invitation_failed', { reason, friendId });
                    socket.emit('debug_push_log', { type: 'error', message: res.error || res.reason });
                }
            })
            .catch(err => {
                console.error('[PUSH] Unexpected error:', err);
                socket.emit('invitation_failed', { reason: 'OFFLINE', friendId });
        ...
        for (const [roomCode, room] of rooms.entries()) {
            ...
            if (playerIndex !== -1) {
                ...
                if (room.gameStarted) {
                    ...
                    // IF NON-HOST LEAVES:
                    // Just mark as disconnected (optional, for now just keeping them is enough for dbId match)
                    console.log(`[Room] Player ${leavingPlayer.name} disconnected from active game ${roomCode} - Keeping slot for reconnection`);

                    io.to(roomCode).emit('notification', {
                        type: 'error',
                        message: `${leavingPlayer.name} a été déconnecté`,
                        timestamp: Date.now()
                    });
